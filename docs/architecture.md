...
- **基本動作**: `quiz`（または VSCode の「クイズを開始」）→ ステージ済み差分から記述式5問を生成（第1〜2問: 前提知識、第3〜5問: 実装が何をしているかの説明）→ ユーザー解答 → 正誤判定 → 不正解時はヒント提示（段階制御）→ 正解/ギブアップ時は解説 → 5問完走でレポートを表示
- **合否の扱い**: 学習支援が目的であり、コミットを罰にしない。**クイズの結果はコミットを左右しない**（全問正解してもコミットはしないし、間違えてもコミットは妨げられない）。ギブアップ・不正解は苦手傾向として記録し、次回の出題に反映する。
- **コミット連携は任意**: 「コミット前に必ず問われる」強制力が欲しい人だけ pre-commit フックを入れる（§3）。既定では git の挙動を一切変えない。
...
└───────────────▲──────────────────────────┘
                │ 発動要求 (HTTP)
    `quiz` / VSCode コマンド / (任意) pre-commit hook
```

### 発動方法

**主インターフェースは明示的な発動で、git とは独立している。**

1. `quiz` コマンド（ターミナル）
2. VSCode コマンド「QQQuestionAI: クイズを開始」（`POST /quiz/start` → Webview パネル）

どちらもステージ済み差分から出題するだけで、**コミットは一切行わない**。

#### ターミナル版が2種類ある理由（`cli.py` と `remote_cli.py`）

| | 実行者 | LLM の呼び方 | API キーの出所 |
|---|---|---|---|
| `cli.py` | `pip install` した開発者 | プロセス内で `create_llm()` | 環境変数 / `backend/.env` |
| `remote_cli.py` | VSCode 拡張の利用者 | 起動済みサーバに HTTP 委譲 | **持たない**（サーバ側にある） |

拡張は API キーを **VSCode SecretStorage** に保存し、バックエンドプロセスへ環境変数として渡す。ターミナルからこの秘密は読めないため、拡張利用者向けの `quiz`（拡張が生成する shim）は `remote_cli` を呼び、キーを持つバックエンドに実行を委譲する。**shim にキーを焼き込まない**ことで、秘密を平文でディスクに置かずに済む（AGENTS.md 安全ルール2）。代償として、拡張利用者の `quiz` は VSCode が開いている間しか使えない。

#### 却下した設計: `git commit -q` の横取り

当初は `~/.zshrc` に `git()` 関数を定義して `-q` 付き commit を横取りしていたが、廃止した:

- `-q` / `--quiet` は **git 本来のフラグ**（コミット要約の抑制）。素直に静かなコミットをしたい人が意図せずクイズに巻き込まれる
- シェル関数の上書きは、そのシェルの**すべての git 呼び出し**（スクリプト・CI・他ツール経由）に影響する副作用が大きすぎる
- 引数を単純走査するため `git commit -m "-q"` のような誤爆がある

回帰防止として `backend/tests/test_install_hook.py` で「シェル設定を書き換えない」「`git commit -q` が素通しされる」ことを検査している。

#### 任意のコミットゲート（pre-commit フック）

強制力が欲しい利用者だけが `scripts/install_quiz_hook.sh` で導入する。

1. フックは **`QQQ_QUIZ=1` と明示された時だけ**発動する（`QQQ_QUIZ=1 git commit -m "..."`）。素の `git commit` や `git commit -q` には影響しない。
2. 発動時はクイズ完走（またはパネルの明示的な中断）まで待機する。完走なら exit 0 でコミット続行、中断なら exit 1 でコミット中止。
3. バックエンド未起動の場合は警告メッセージだけ出して素通しする。クイズが受けられないことを理由にコミットを壊さない。

### データフロー

1. `quiz` / VSCode コマンド（または任意のフック）→ `POST /quiz/start` を呼ぶ
...
8. 5問終了 → `evaluator` がレポート出力、`learner_model` が履歴を `history.jsonl` へ追記（フック経由で発動していた場合のみ、完走を通知しコミット続行）
...
5. **発動導線**: `quiz` コマンド + VSCode コマンド。（任意）pre-commit フックによるコミットゲート
